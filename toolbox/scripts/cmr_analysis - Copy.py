    ...
    try:
        ...
        try:
            ...
            if myStudy.hazardElementIntersection():
                ...
                earLyrName = "ear_{0}".format(studyId)
                arcpy.MakeFeatureLayer_management(myStudy.outputFeatures,earLyrName)
                arcpy.ApplySymbologyFromLayer_management(earLyrName, os.path.join(templateDir, "ear.lyr"))
                earLyr = mapping.Layer(earLyrName)
                earLyr.definitionQuery = '"elementAtRisk" <> 0'.format()
                mapping.AddLayer(df, earLyr)

##                earLyrName = "damageByElement_{0}".format(studyId)
##                arcpy.MakeFeatureLayer_management(myStudy.outputFeatures,earLyrName)
##                tblSrc = os.path.join(sdeConnFile, "cmrV_DamagesByElement")
##                tblViewName = "damageByElementTbl_{0}".format(studyId)
##                strCond = "study_id = {0}".format(studyId)
##                arcpy.MakeTableView_management(tblSrc, tblViewName, strCond)
##                arcpy.AddJoin_management(earLyrName,"OBJECTID",tblViewName,"element_feature_id","KEEP_ALL")
##                arcpy.ApplySymbologyFromLayer_management(earLyrName, os.path.join(templateDir, "damageByElement.lyr"))
##                earLyr = mapping.Layer(earLyrName)
##                mapping.AddLayer(df, earLyr)
